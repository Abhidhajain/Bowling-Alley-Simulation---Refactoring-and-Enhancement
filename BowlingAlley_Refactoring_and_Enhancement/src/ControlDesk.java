...
class ControlDesk extends Thread implements Serializable{

	private HashSet lanes;			/** The collection of Lanes */
	private Vector partyQueue;		/** The party wait queue */
	private int numLanes;			/** The number of lanes represented */
	public Vector subscribers; 	/** The collection of subscribers */
	...
	public void assignLane() {
		...
		while (it.hasNext() && partyQueue.size()!=0) {
			...
			if (curLane.scoreCalculation.isPartyAssigned() == false) {
				System.out.println("ok... assigning this party");
				((Party) partyQueue.remove(0)).assignParty(curLane);
			}
		}
		ControlDeskSubscribe.publish(this);
	}
	...
	public Vector getPartyQueue() {
		...
		for ( int i=0; i < ( (Vector)partyQueue).size(); i++ ) {
			String nextParty =
